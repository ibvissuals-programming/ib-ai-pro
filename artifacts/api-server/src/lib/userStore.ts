...
export async function repairCeoAccount(): Promise<void> {
  ...
  if (!existing) {
    // Memory is cold: PG load failed at boot and JSON fallback had no CEO record.
    // ALWAYS check PG before creating anything — never overwrite an existing DB hash.
    if (isPostgresEnabled()) {
      try {
        const pgCeo = await pgGetUserByUsername(ceoUsername);
        if (pgCeo) {
          // CEO exists in PG — sync into memory only, never touch password_hash.
          store.set(pgCeo.id, {
            id:           pgCeo.id,
          ...
          scheduleSave(); // write corrected record back to JSON fallback file
          _ceoBootstrapState = { ready: true, autoCreated: false, tempPassword: null };
          logger.info(
            { username: ceoUsername, id: pgCeo.id },
            "[ceoRepair] CEO synced from PG into memory (was absent from JSON fallback) — password_hash untouched",
          );
          return;
        }
      } catch (pgErr) {
        // PG unreachable — cannot safely determine whether CEO already exists.
        // Refuse to proceed rather than risk overwriting a valid DB record with a random hash.
        logger.error(
          { pgErr, username: ceoUsername },
          "[bootstrap] FATAL: PG unreachable during CEO existence check — refusing to bootstrap with unknown credentials. Fix PG connectivity and restart.",
        );
        _ceoBootstrapState = { ready: false, autoCreated: false, tempPassword: null };
        return;
      }
    }

    // CEO does not exist in PG (or PG is disabled).
    // Only create if CEO_PASSWORD is explicitly set — never auto-generate a random password.
    if (!ceoPassword) {
      logger.error(
        { username: ceoUsername },
        "[bootstrap] FATAL: CEO account does not exist and CEO_PASSWORD env var is not set. " +
        "Add CEO_PASSWORD to Replit Secrets and restart to create the account deterministically.",
      );
      _ceoBootstrapState = { ready: false, autoCreated: false, tempPassword: null };
      return;
    }

    // Fully deterministic creation: CEO_PASSWORD is set and no CEO exists anywhere.
    const user: User = {
      id:           randomUUID(),
      username:     ceoUsername,
    ...
    usernameIndex.set(user.username, user.id);
    await persistStore();
    _ceoBootstrapState = { ready: true, autoCreated: true, tempPassword: null };
    logger.info({ username: ceoUsername }, "[ceoRepair] CEO account created with CEO_PASSWORD");
    return;
  }
