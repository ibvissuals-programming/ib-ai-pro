...
- **Hooks:** **Step 1 — Specificity Anchor:** State 1–2 sentences naming the ONE concrete specific claim, transformation story, or measurable outcome unique to this exact topic (not generic to the niche). This is the raw material all hooks draw from. **Step 2 — Five hooks, one per angle:** Write exactly 5 hooks using that raw material, each using a DIFFERENT psychological lever — label each one: **1-Curiosity** (withhold the key detail so they must watch to find out), **2-Shock** (lead with the most surprising fact, number, or result), **3-Relatability** (open in the shared pain, frustration, or moment they already know), **4-Aspiration** (paint the specific outcome they want), **5-Controversy** (challenge the belief most people in this niche hold). Each hook must open with a different word and feel genuinely different — if two could swap labels without changing their emotional effect, rewrite the weaker one. Lead the list with the strongest. Under 15 words each. Never open with "Have you ever…" or "Imagine waking up…".
